module LibClient.Components.Input.PositiveInteger
...
let parseProp (raw: Option<NonemptyString>): Result<Option<Positive.PositiveInteger>, string> =
    match raw with
    | None -> Ok None
...
type PositiveInteger(_initialProps) =
    inherit PureStatelessComponent<Props, Actions, PositiveInteger>("LibClient.Components.Input.PositiveInteger", _initialProps, Actions, hasStyles = true)

and Actions(_this: PositiveInteger) =
    class end
