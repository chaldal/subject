...
[<AutoOpen>]
module Input_UnsignedDecimalTypeExtensions =
    type LibClient.Components.Constructors.LC.Input with
        static member UnsignedDecimal(value: Value, validity: InputValidity, onChange: Value -> unit, ?children: ReactChildrenProp, ?requestFocusOnMount: bool, ?label: string, ?placeholder: string, ?prefix: string, ?suffix: InputSuffix, ?tabIndex: int, ?onKeyPress: (Browser.Types.KeyboardEvent -> unit), ?onEnterKeyPress: (ReactEvent.Keyboard                -> unit), ?styles: array<ViewStyles>, ?key: string, ?xLegacyStyles: List<ReactXP.LegacyStyles.RuntimeStyles>) =
            let __props =
                {
                    Value = value
                    Validity = validity
            ...
            LibClient.Components.Input.UnsignedDecimal.Make
                __props
