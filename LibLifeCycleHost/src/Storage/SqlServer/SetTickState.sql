CREATE OR ALTER PROCEDURE [___SCHEMA_NAME___].[___LIFECYCLE_NAME____SetTickState_V2] (
    @id                         NVARCHAR(80),
...
AS
    ...
    IF @id IS NULL
        RAISERROR('___LIFECYCLE_NAME____SetTickState_V2 Param id should not be NULL', 11, 1);

    IF @nextSideEffectSeqNumber IS NULL AND ((SELECT COUNT(*) FROM @newSideEffects) > 0)
        RAISERROR('___LIFECYCLE_NAME____SetTickState_V2 Param nextSideEffectSeqNumber should not be NULL if there are any new side-effects', 11, 1);

    IF @concurrencyToken IS NULL
        RAISERROR('___LIFECYCLE_NAME____SetTickState_V2 Param concurrencyToken should not be NULL for existing state', 11, 1);
    ...
    BEGIN TRY
        ...
        BEGIN
            DECLARE @setNextTickOutputTable TABLE (
            ...
            UPDATE [___SCHEMA_NAME___].[___LIFECYCLE_NAME___]
                ...
                INTO
                    @setNextTickOutputTable (ConcurrencyToken, Version)
                WHERE
