...
module With_SubjectsWithCountTypeExtensions =
    type LibUiSubject.Components.Constructors.UiSubject.With with
        static member SubjectsWithCount(service: LibUiSubject.Services.SubjectService.ISubjectService<'Subject, 'Projection, 'Id, 'Index, 'Constructor, 'Action, 'Event, 'OpError>, by: By<'Subject, 'Projection, 'Id, 'Index, 'Action, 'Event, 'OpError>, ``with``: WithSubjectsWithCount<'Id, 'Projection>, ?children: ReactChildrenProp, ?useCache: UseCache, ?key: string, ?xLegacyStyles: List<ReactXP.LegacyStyles.RuntimeStyles>) =
            ...
            LibUiSubject.Components.With.SubjectsWithCount.Make
                __props
