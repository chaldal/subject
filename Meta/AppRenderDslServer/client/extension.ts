...
export function activate(context: ExtensionContext) {
	...
	if (debugMode) {
		serverOptions = {
			...
			args: ['run', '-c', 'Debug', '--project', 'src/RenderDSLServer'],
			transport: TransportKind.stdio,
	...
	let clientOptions: LanguageClientOptions = {
		...
		documentSelector: [{scheme: 'file', language: 'renderdsl'}],
		synchronize: {
			// Synchronize the setting section 'languageServerExample' to the server
			configurationSection: 'renderDsl',
	...
	let client = new LanguageClient('renderDsl', 'Render DSL Server', serverOptions, clientOptions);
	...
	commands.registerCommand('render.command.test.run', runTest);
	commands.registerCommand('render.command.test.debug', debugTest);
	commands.registerCommand('render.command.goto', goto);
}
...
function runTest(projectPath: string, fullyQualifiedName: string): Thenable<TaskExecution> {
	...
	let task = new Task(kind, workspaceFolder, 'RenderDSL Test', 'RenderDSL Language Server', shell)
...
function binName(): string {
	var baseParts = ['src', 'RenderDSLServer', 'bin', 'Release', 'netcoreapp2.0'];
...
function getPathParts(platform: string): string[] {
	switch (platform) {
		case 'win32':
			return ['win10-x64', 'publish', 'RenderDSLServer.exe'];

		case 'linux':
			return ['linux-x64', 'publish', 'RenderDSLServer'];

		case 'darwin':
			return ['osx.10.11-x64', 'publish', 'RenderDSLServer'];
	}
