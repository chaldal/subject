...
let connect(serverFactory: ILanguageClient -> ILanguageServer, receive: BinaryReader, send: BinaryWriter) =
    ...
    let processRequest(request: Request): Async<string option> =
        dprintfn "XXXXX getting a request %O" request
        match request with
        | Initialize(p) ->
    ...
    Thread(fun () ->
        try
            ...
            for m in readMessages(receive) do
                // NOTE by anton
                // initially they didn't have a try/with for each message, so the whole
                // thing would break down from one QQQ. Seeing if having the
                // try/with inside the for loop will fix that issue
                try
                ...
                with e ->
                    dprintfn "Exception in read thread on message %O\n\n%O" m e
    ...
    while not quit do
        dprintfn "Waiting for the next message on processQueue"
