...
module BaseTypeExtensions =
    type ThirdParty.ImagePicker.Components.Constructors.ImagePicker with
        static member Base(value: list<File>, validity: InputValidity, onChange: Result<list<File>, string> -> unit, ?children: ReactChildrenProp, ?showPreview: bool, ?selectionMode: SelectionMode, ?maxFileCount: Positive.PositiveInteger, ?maxFileSize: int<KB>, ?styles: array<ViewStyles>, ?key: string, ?xLegacyStyles: List<ReactXP.LegacyStyles.RuntimeStyles>) =
            let __props =
                {
                    Value = value
                    Validity = validity
            ...
            ThirdParty.ImagePicker.Components.Base.Make
                __props
